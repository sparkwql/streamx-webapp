<script lang="ts" setup>
  ...
  const clusters = ref<FlinkClusterResponse[]>([]);
  ...
  function handleAdd() {
    router.push({
      path: '/flink/setting/add_cluster',
    });
  }
  ...
  /* 前往编辑集群 */
  function handleEditCluster(item: FlinkClusterResponse) {
    router.push({
      path: '/flink/setting/edit_cluster',
      params: {
        id: item.id,
      },
    });
  ...
  /* 部署 */
  async function handleDeployCluser(item: FlinkClusterResponse) {
  ...
  /* 删除 */
  async function handleDelete(item: FlinkClusterResponse) {
  ...
  /* shutdown */
  async function handleShutdownCluster(item: FlinkClusterResponse) {
  ...
  <List>
    <ListItem v-for="(item, index) in clusters" :key="index">
      ...
      <template #actions>
        ...
        <Tooltip title="View Cluster Detail">
          <a-button
