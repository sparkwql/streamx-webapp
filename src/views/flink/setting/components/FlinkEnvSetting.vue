<script lang="ts" setup>
  ...
  /* 查看配置 */
  async function handleFlinkConf(item: FlinkEnvResponse) {
    const res = await fetchFlinkInfo(item.id);
    openEnvDrawer(true, res);
  }

  /* 设为默认环境 */
  async function handleSetDefault(item: FlinkEnvResponse) {
    if (item.isDefault) {
...
<template>
  ...
  <List>
    <ListItem v-for="(item, index) in flinks" :key="index">
      ...
      <template #actions>
        <a-button type="link" @click="handleEditFlink(item)">Edit</a-button>
        <a-button type="link" @click="handleFlinkConf(item)">Flink Conf</a-button>
      </template>
    </ListItem>
  </List>
