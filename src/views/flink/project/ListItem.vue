...
<template>
  <a-list-item>
    <a-list-item-meta class="item-meta">
      ...
      <template #description>
        <a-popover arrow-point-at-center trigger="hover">
          <template #content>{{ item.url }}</template>
    ...
    <div class="operation">
      <a-tooltip title="See Build log" v-if="isBuilding">
        <a-button shape="circle" @click="seeLog"
          ><a-icon spin type="sync" style="color: #4a9ff5"
        /></a-button>
      </a-tooltip>

      <a-tooltip title="Build Project" v-if="!isBuilding" v-auth="'project:build'">
        <a-popconfirm
  ...
  <log-modal :id="item.id" :visible="true" />
</template>
<script lang="ts" setup>
  import { List, Badge, Tag, Tooltip, Popconfirm } from 'ant-design-vue';
